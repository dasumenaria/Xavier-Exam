...
<div class="a1">
    ...
    <table width="100%"  cellspacing="0px" height="300" cellpadding="0px" border="1" id="sample_1">
					...
					<script>
						$(document).ready(function()
						{
							var o=<?php echo $l; ?>;
								if (o % 2 == 0){
								var p=o-1;
								var g_tot_max_mark=$("#AllOverTotaltext_"+p).val();
								var g_tot_gain=$("#AllOverTotaltext_"+o).val();
								var per=((g_tot_gain/g_tot_max_mark)*100);
								
								$("#per_"+o).html(per.toFixed(2));
								}
 								//$("#AllOverTotal_<?php echo $b;?>").text(totmarks);
						});
						</script>
						<?php
					}
		...
		<tfoot>
		<tr class="header_font">
             <th height="33" colspan="2" style="text-align:right;">Percentage &nbsp; &nbsp;</th>
				<?php 
				$m=2;
                $st=mysql_query("select DISTINCT(term_id) from `master_architecture` where `marksheet_term_id`='$term_id' && `class_id`='$class_id' && `section_id`='$section_id'");
                while($ft=mysql_fetch_array($st))
                {
					...
					?>
					<th height="25px" colspan="<?php echo $colspan+2; ?>" ><b id="per_<?php echo $m; ?>"> </b> %</th>
					<?php
					$m=$m+2;
                }
                ?>	
            <th colspan="2" ><b id="per_<?php echo $m; ?>"> </b> %</th>
         </tr>
		 <tr class="header_font">
             <th height="33" colspan="2" style="text-align:right;">Division &nbsp; &nbsp;</th>
				<?php 
                $st=mysql_query("select DISTINCT(term_id) from `master_architecture` where `marksheet_term_id`='$term_id' && `class_id`='$class_id' && `section_id`='$section_id'");
                while($ft=mysql_fetch_array($st))
                {
					...
					if($colspan==1){$colspan=0;}
					?>
					<th height="25px" colspan="<?php echo $colspan+2; ?>"><b></b></th>
					<?php
                }
                ?>	
            <th colspan="2"></th>
		...
		<tr class="header_font">
             <th height="33" colspan="2" style="text-align:right;">Rank &nbsp; &nbsp;</th>
				<?php 
                $st=mysql_query("select DISTINCT(term_id) from `master_architecture` where `marksheet_term_id`='$term_id' && `class_id`='$class_id' && `section_id`='$section_id'");
                while($ft=mysql_fetch_array($st))
                {
...
//--- Grading Subject
		...
		while($FtcSubjectDataQuery=mysql_fetch_array($SubjectDataQuery))
		{
			...
			while($ftc_subject=mysql_fetch_array($FindSubject))
			{
				...
				?>
                 <tr>
                    <th height="33" <?php if(empty($sub_subject_name)){ ?>colspan="2" <?php }else{ ?> <?php } ?> width="8%" style="margin-left:5px"><?php echo $subject; ?></th>
					
					<?php if(!empty($sub_subject_name)){ ?>
					<th height="33" width="8%" style="margin-left:5px"><?php if(!empty($sub_subject_name)){ echo $sub_subject_name; } ?></th>
					<?php
					}
					$TotalMaxMarks=0;
					$TotalGetMarks=0;
 				//* Architacher Loop
					...
					while($ftc_ArchitacherQuery=mysql_fetch_array($ArchitacherQuery))
					{
						...
						while($ftc_categorywise=mysql_fetch_array($category_wisecolumn))
						{
							...
							while($exam_category_Fetch=mysql_fetch_array($exam_category_query))
							{
								...
								?>
								<th colspan="2"><?php //echo $TotalGetMarks.'( '.$TotalOneSubject.' )'; 
									echo $grade_find=calculate_all_grads($TotalOneSubject); 
								?> </th> 
							<?php
							}
 						}
					...
					//** END Exam Mapping Table ------- FInd Exam Category 
 					?>
                         	<th colspan="2"><?php 
							$ovr_all_gt_marks=round(($OverAllTotalGetMarkss/$OverAllTotalMaxMarkss)*10);
							echo $grade_find=calculate_all_grads($ovr_all_gt_marks); 
								?> </th> 
                    <?php
						//** FInd Grade
						if($TotalGetMarks==0 || $TotalMaxMarks==0){$GetOneSubjectPercentage=0;}
        ...
        <!----End Maximum Rows---->
		 <br/><br/><br/><br/><br/><br/><br/>
			...
			<?php
				...
				?>
							 <th style="height:55px" width="15%">Status : </th>
							 <th width="25%"><?php echo $status; ?></th>
							 
							 <th width="15%">Result : </th>
							 <th width="15%"><?php echo $FinalStatusOfResult; ?></th>
							...
							?>
							<th width="15%"><?php echo $totalAttendP.'/'.$totalAttendPGET;?></th>
							 
						 </tr>
 						 <tr>
							<th style="height:55px" >Remarks : </th>
